...
import { latexToOmml } from "./mathmlToOmml.ts";
import { xmlSafe } from "./xmlText.ts";
import type { DiagramImage, PictureSource, ReferencedImage } from "./images.ts";
...
/** The numbering definition ordered lists are drawn from; declared by `docxDocument`. */
 ...
 */
async function renderDiagrams(root: Root, render: PictureSource["renderDiagram"]): Promise<Map<Code, DiagramImage>> {
  const drawn = new Map<Code, DiagramImage>();
  if (render === undefined) return drawn;
  const fences: Code[] = [];
  ...
  for (const [index, fence] of fences.entries()) {
    try {
      drawn.set(fence, await render(fence.value, `docx-export-${index}`));
    } catch {
...
/** How many mermaid fences the document holds — the ones a caller without a renderer leaves as code. */
export function countDiagrams(text: string): number {
  let count = 0;
  const walk = (node: { type: string; children?: unknown[] }) => {
    if (node.type === "code" && isDiagram(node as Code)) count++;
    for (const child of (node.children ?? []) as { type: string; children?: unknown[] }[]) walk(child);
  };
 ...
 */
async function loadReferences(root: Root, source: PictureSource): Promise<Refs> {
  const refs: Refs = new Map();
  const { imageKey, loadImage } = source;
  if (imageKey === undefined || loadImage === undefined) return refs;
  const nodes: Image[] = [];
  const walk = (node: { type: string; children?: unknown[] }) => {
  ...
  // Keyed by what the reference resolves to, and holding the misses as well as the
  // hits: a file that is not there must not be asked for once per mention.
  const byKey = new Map<string, ReferencedImage | undefined>();
  for (const node of nodes) {
    const key = imageKey(node.url);
    // Not ours to fetch — an absolute URL, a data URI, a protocol-relative host.
    if (key === undefined) continue;
    if (!byKey.has(key)) byKey.set(key, await loadImage(node.url));
    const picture = byKey.get(key);
 ...
 * references loaded — which happens once up front rather than during the walk:
 * the mapping itself stays a plain recursive function over the tree, which is
 * much easier to reason about than one that awaits inside itself. Who produces
 * them, and how, is the caller's (`PictureSource`).
 */
export async function markdownToDocx(text: string, source: PictureSource = {}): Promise<DocxBlock[]> {
  const root = parseMarkdown(text);
  const diagrams = await renderDiagrams(root, source.renderDiagram);
  const images = await loadReferences(root, source);
