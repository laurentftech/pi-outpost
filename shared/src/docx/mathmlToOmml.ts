...
import { findMathNode, hasClass, parseMathMarkup, type MathNode } from "./mathTree.ts";
 ...
 * source. Either way the export survives one bad equation. Read with `mathTree`
 * rather than the browser's `DOMParser`, so the server writes the same equations.
 */
export function mathTreeFor(latex: string, displayMode: boolean): MathNode {
  const markup = katex.renderToString(latex, {
    output: "mathml",
    displayMode,
    throwOnError: false,
  });
  const math = findMathNode(parseMathMarkup(markup), (node) => node.tagName.toLowerCase() === "math");
  if (math === undefined) throw new UnsupportedMathError("math");
  return math;
}

/** Element children only — MathML carries no meaningful text between elements. */
function elementChildren(node: MathNode): MathNode[] {
  return [...node.children];
}

/** The text a leaf carries, with the whitespace MathML pads operators with removed. */
function leafText(node: MathNode): string {
  return (node.textContent ?? "").replace(/[\u2061-\u2064]/g, "").trim();
}

function convertAll(nodes: readonly MathNode[]): MathComponent[] {
 ...
 */
function convert(node: MathNode): MathComponent[] {
 ...
 */
function naryFrom(node: MathNode, following: readonly MathNode[]): MathComponent[] | undefined {
 ...
 */
function groupBrackets(nodes: readonly MathNode[]): MathComponent[] {
 ...
 */
function convertRow(nodes: readonly MathNode[]): MathComponent[] {
...
export function latexToOmml(latex: string, displayMode: boolean): MathComponent[] {
  const math = mathTreeFor(latex, displayMode);
  ...
  if (findMathNode([math], (node) => hasClass(node, "katex-error")) !== undefined) throw new UnsupportedMathError("katex-error");

  const semantics = findMathNode(math.children, (node) => node.tagName.toLowerCase() === "semantics");
  const top = semantics === undefined ? elementChildren(math) : elementChildren(semantics);
