...
signature described below satisfies Apple Silicon's kernel-level requirement that a
binary carry *some* signature to run at all — a different check from Gatekeeper's,
and it does not satisfy it. Gatekeeper acts on the quarantine attribute a browser or
`curl` sets on anything downloaded from the internet, unrelated to the `chmod` above,
and it is not a dismiss-and-continue warning on first launch:
...
- **Windows** SmartScreen shows "Windows protected your PC" with only **Don't run**
  visible; click **More info**, then **Run anyway**. A managed or corporate machine
  may have that button removed by policy — there is no user-side bypass then.
