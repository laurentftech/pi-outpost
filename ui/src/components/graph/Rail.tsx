import { laneColor, laneX, railWidth, ROW_H, type RailRow } from "./lanes";
 ...
 */
export function Rail({ row, laneCount }: { row: RailRow; laneCount: number }) {
  ...
  return (
    <svg width={width} height={ROW_H} viewBox={`0 0 ${width} ${ROW_H}`} aria-hidden className="shrink-0">
      {row.through.map((lane) => (
        <line key={lane} x1={laneX(lane)} y1={0} x2={laneX(lane)} y2={ROW_H} stroke={laneColor(lane)} strokeWidth={2} opacity={0.55} />
      ))}
