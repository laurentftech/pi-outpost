...
Optional shared-secret token authentication for the WebSocket and HTTP API, with a browser
token flow (URL parameter, localStorage, token screen) and an embed token option.
