...
- [ ] 4.10 Carry a proposed table through the approval and handover path a graph already has: derive each row's mark from the proposal, show the target artifact and the revision it was read at, and hand back the validated document unchanged for the tool that writes it back.
