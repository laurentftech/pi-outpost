...
### 1. Model a resource inventory before modeling Git state
...
### 2. Keep resource Git in a separate service
...
### 3. Represent assessment as a revision-bound snapshot
...
### 4. Treat extension-bearing repositories as executable units
...
### 5. Coordinate updates with workspace lifecycle
...
### 6. Use the repository-first dialog from prototype A

Settings gains one Agent resources entry point opening a dialog large enough for a split view. The left pane shows repository and non-updateable pseudo-groups, counts, status badges, search, kind filters, and attention filtering. The right pane shows branch/upstream state, actions, status explanation, and separate skill/extension lists. Existing Settings path editors remain the place to add or remove roots.
...
### 7. Degrade explicitly for runtime capability gaps
...
- [Git fetch can invoke configured credential or transport helpers] → Run only after explicit user refresh/update intent, inherit the deployment's existing Git authentication policy, bound execution, and document this operational trust boundary.
...
3. Enable read-only repository discovery and assessment, then ship the dialog with updates feature-gated off until update and lifecycle tests pass.
