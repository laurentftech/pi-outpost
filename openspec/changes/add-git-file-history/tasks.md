...
- [x] 7.4 Implement selection: hover/focus-revealed `base` and `target` ghost buttons using `TreeMenu`'s `ACTION_CLASS`; a plain row selection sets target and, with no base set, takes the first parent as base; landing a role on the row holding the other role swaps them (a silent refusal read as a broken button when driving the real app); header `swap ⇄` and `clear`.
...
- [x] 9.2 Run `openspec validate --changes add-git-file-history --strict` (passes) and `check_spec_drift`. It found real drift the delta did not cover: `conversation-tree`'s BranchGraphRendering still claimed the current path sits on the first lane — the very property the tree fix removes — so that main spec was corrected directly, and a `components` delta was added for `GitFileHistory` and the shared lane primitives.
- [x] 9.3 Record the three decisions via `record_decision` (unblocked once the openlore MCP server was re-wired with `--preset full`): the stitched `--full-history` + `--follow`-probe log strategy, the parent-pruning fallback rule, and the shared rail extraction.
