...
- Preserve sandbox/resource coherence when a local or Git-backed root is activated: the replacement session must retain configured resource roots as read-only exceptions even outside the sandbox, and a replacement refused before session handoff must restore the prior persisted settings, resources, and tool boundary instead of acknowledging a split state.
