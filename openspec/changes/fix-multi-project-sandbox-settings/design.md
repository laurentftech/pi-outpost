...
- **The server's own project is the booted one** (`serverProject`). Roots are editable only from it and only while it is alone (`rootEditableFrom`); otherwise the request's roots are replaced by the current ones before anything else, so a stale or forged root cannot move them.
...
- [The server's project closed while others stay open] → no project can edit the roots until it is reopened alone; acceptable, and the panel still shows each project's own directory.
